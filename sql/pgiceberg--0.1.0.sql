...
-- PostgreSQL and Iceberg commits are not a single atomic unit. Iceberg is
-- published at PRE_COMMIT with a durable recovery log and commit identifier.
-- pgiceberg.repair_commit can roll back or acknowledge in-doubt Iceberg
-- snapshots after a PostgreSQL abort or crash. Cross-table Iceberg commits
-- are sequential, not atomic; see docs/design/postgres-iceberg-commit.md.
CREATE FUNCTION pgiceberg.commit_recovery_log()
RETURNS TABLE (
  commit_id text,
  state text,
  postgres_xid text,
...
AS 'MODULE_PATHNAME', 'pgiceberg_commit_recovery_log'
LANGUAGE C
SET search_path = pg_catalog, pgiceberg;

COMMENT ON FUNCTION pgiceberg.commit_recovery_log() IS
  'List durable pgiceberg recovery logs for Iceberg commits that have not yet been reconciled with PostgreSQL transaction outcome.';

CREATE FUNCTION pgiceberg.reconcile_commits()
RETURNS TABLE (
  commit_id text,
  state text,
  verdict text,
...
AS 'MODULE_PATHNAME', 'pgiceberg_reconcile_commits'
LANGUAGE C
SET search_path = pg_catalog, pgiceberg;
...
RETURNS text
AS 'MODULE_PATHNAME', 'pgiceberg_repair_commit'
LANGUAGE C STRICT SECURITY DEFINER
SET search_path = pg_catalog, pgiceberg;
...
CREATE FUNCTION pgiceberg.rollback_iceberg_snapshot(
  name text,
  namespace text,
  table_name text,
  snapshot_id bigint
...
RETURNS void
AS 'MODULE_PATHNAME', 'pgiceberg_rollback_iceberg_snapshot'
LANGUAGE C STRICT SECURITY DEFINER
SET search_path = pg_catalog, pgiceberg;
...
CREATE FUNCTION pgiceberg.current_xact_commit_id()
RETURNS text
AS 'MODULE_PATHNAME', 'pgiceberg_current_xact_commit_id'
LANGUAGE C
SET search_path = pg_catalog, pgiceberg;

COMMENT ON FUNCTION pgiceberg.current_xact_commit_id() IS
  'Return the pgiceberg commit identifier for pending Iceberg DML in the current PostgreSQL transaction, or NULL when this backend has no pending writes.';
